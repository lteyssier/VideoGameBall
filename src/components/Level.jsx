...
const floor1Material = new THREE.MeshStandardMaterial({color:'#1e69de',metalness:'0.1'})
const floor2Material =  new THREE.MeshStandardMaterial({color:'#32aff8'})
const obstacleMaterial =  new THREE.MeshStandardMaterial({color:'#ff1c9c'})
const wallMaterial =  new THREE.MeshStandardMaterial({color:'#ffcf00'})
...
export function BlockEnd({position=[0,0,0]}){
    const crown  = useGLTF('./crown.glb')

    crown.scene.children.forEach((mesh) =>
    {
...
function Bounds({length=1}){
    return<>
        <RigidBody type='fixed' restitution={0.2} friction={0}>
            <mesh
                position={[2.2,0.5, -(length*2)+2]}
                geometry={boxGeometry}
                material={wallMaterial}
                scale={[0.4,1,4*length]}
                castShadow
            />
            <mesh
                position={[-2.2,0.5, -(length*2)+2]}
...
export const Level = ({count = 10, types = [BlockSpinner, BlockAxe, BlockLimbo], seed=0}) => {
